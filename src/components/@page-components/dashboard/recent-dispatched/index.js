...
const RecentDispatched = () => {
    const { dispatchedClaims } = useSelector(({ claims }) => claims);
    ...
    return (
        <>
            <DMTCard
                title={'Recently Dispatched Services'}
                actions={
                    <NextLink style={{ textDecoration: 'none' }} href={'/dashboard/dispatched-services'} passHref={true}>
            ...
            >
                    <List disablePadding dense>
                        {approvalData.length > 0 ? showRecords(approvalData).map( (claim, index) => (
                            ...
                            <>
                                <Typography>
                                    {isLoading ? "Fetching dispatched services..." : "No recently dispatched services at the moment."}
                                </Typography>
                            </>
                        )}
                    </List>
